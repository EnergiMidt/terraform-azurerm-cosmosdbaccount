...
| <a name="input_configuration"></a> [configuration](#input\_configuration) | (Optional) The configuration for block type arguments. | `any` | <pre>{<br>  "consistency_policy": {<br>    "consistency_level": "Session",<br>    "max_interval_in_seconds": 5,<br>    "max_staleness_prefix": 100<br>  },<br>  "geo_location": {<br>    "primary_geo_location": {<br>      "failover_priority": 0,<br>      "location": "AzureLocation",<br>      "zone_redundant": false<br>    }<br>  }<br>}</pre> | no |
